 ...
 */
class PlainDomainUserTest {

  PlainDomainUser target;

  @BeforeEach
  void init() {
    target = new PlainDomainUser();
  }
   ...
   *
   ...
   List<MemberOf> values = Collections.singletonList(MemberOf.builder()
   .distinguishedName(value)
   .name(value)
   .build());
   DomainUser model = new DomainUser();
   model.setMembership(values);
   assertEquals(values, model.getMembership());
   ...
   *
   @Test void getUnixHomeDirectory() {
   String value = UUID.randomUUID().toString();
   DomainUser model = new DomainUser();
